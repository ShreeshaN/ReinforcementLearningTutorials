...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        self.duration = 0
        self.episode = 0
        self.last_100_reward = deque([], args.capture_window)
        if not os.path.exists(self.save_network_path):
            os.makedirs(self.save_network_path)
        if not os.path.exists(self.log_path):
            os.makedirs(self.log_path)
    ...
    def run(self, state, action, reward, terminal, observation):
        ...
        if self.t >= self.initial_replay_size:
            ...
            if self.t % self.save_interval == 0:
                save_path = self.save_network_path + '/' + self.run_name + '_' + str(self.t) + '.pt'
                torch.save(self.q_network.state_dict(), save_path)
                print('Successfully saved: ' + save_path)

        self.total_reward += reward
        self.total_q_max += np.max(
                self.q_network(tensor(state).unsqueeze(0)).detach().cpu().numpy())
        self.duration += 1
        ...
        if terminal:
            ...
            print(
                    'EPISODE: {0:6d} / TIMESTEP: {1:8d} / DURATION: {2:5d} / EPSILON: {3:.5f} / AVG_REWARD: {4:2.3f} / AVG_MAX_Q: {5:2.4f} / AVG_LOSS: {6:.5f} / MODE: {7}'.format(
                            ...
                            np.mean(self.last_100_reward), self.total_q_max / float(self.duration),
                                                           self.total_loss / (float(self.duration) / float(
                                                                   self.train_interval)), mode), file=self.log)
    ...
    def log_summary(self, global_step):
        ...
        self.writer.add_scalar('Train/Average Loss', self.total_q_max / float(self.duration),
                               global_step) if self.duration > 0 else None
        self.writer.add_scalar('Train/Average Q',
    ...
    def train_network(self):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, terminal_batch = tensor(state_batch).to(
                self.device), tensor(action_batch).to(self.device), tensor(reward_batch).to(self.device), tensor(
                next_state_batch).to(self.device), tensor(terminal_batch).to(self.device)
        # Q value from target network
        # target_q_values_batch = self.target_network.predict([list2np(next_state_batch), self.dummy_batch])[0]
        target_q_values_batch = self.target_network(next_state_batch).detach()

        # create Y batch depends on dqn or ddqn
        # if self.ddqn:
        #     next_action_batch = np.argmax(self.q_network(next_state_batch), dim=-1)
        #     for i in range(self.batch_size):
        #         y_batch.append(reward_batch[i] + (1 - terminal_batch[i]) * self.gamma * target_q_values_batch[i][
        #             next_action_batch[i]])
        #     y_batch = tensor(y_batch).to(device=self.device)
        # else:

        y_batch = reward_batch + (1 - terminal_batch) * self.gamma * torch.max(target_q_values_batch, dim=-1).values
        ...
        current_q_values = self.q_network(state_batch).gather(1, action_batch.unsqueeze(1).long()).squeeze(1)
        loss = F.mse_loss(current_q_values, y_batch)
        self.optimizer.zero_grad()
        loss.backward()
        for param in self.q_network.parameters():
            param.grad.data.clamp_(-1, 1)
